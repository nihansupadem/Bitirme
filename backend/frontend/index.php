...
<title>OpTrade — BIST30 Financial Forecasting</title>
...
<div class="ticker-bar">
  <div class="ticker-inner" id="ticker">
    <?php
    $tickers=[['YKBNK','+1.2%','up'],['GARAN','+0.8%','up'],['THYAO','-0.5%','down'],
...
<div class="selector-wrap">
  <div class="selector-card">
    <label>Select a BIST30 Stock</label>
    <div class="select-row">
      <select id="stockSelect">
        ...
        $stocks=[
          ...
          'BIMAS'=>'BIMAS - BIM','EKGYO'=>'EKGYO - Emlak Konut','ENKAI'=>'ENKAI - Enka Insaat',
          'EREGL'=>'EREGL - Eregli Demir','FROTO'=>'FROTO - Ford Otosan',
          'GARAN'=>'GARAN - Garanti BBVA','HALKB'=>'HALKB - Halkbank',
          'ISCTR'=>'ISCTR - Is Bankasi','KCHOL'=>'KCHOL - Koc Holding','KOZAL'=>'KOZAL - Koza Altin',
          'KRDMD'=>'KRDMD - Kardemir','MGROS'=>'MGROS - Migros','ODAS'=>'ODAS - Odas Elektrik',
          'PETKM'=>'PETKM - Petkim','PGSUS'=>'PGSUS - Pegasus',
          'SAHOL'=>'SAHOL - Sabanci Holding','SASA'=>'SASA - SASA Polyester',
          'SISE'=>'SISE - Sisecam','TAVHL'=>'TAVHL - TAV Havalimanlari','TCELL'=>'TCELL - Turkcell',
          'THYAO'=>'THYAO - Turkish Airlines','TKFEN'=>'TKFEN - Tekfen Holding','TOASO'=>'TOASO - Tofas',
          'TTKOM'=>'TTKOM - Turk Telekom','TUPRS'=>'TUPRS - Tupras','ULKER'=>'ULKER - Ulker Biskuvi',
          'VAKBN'=>'VAKBN - Vakifbank','VESTL'=>'VESTL - Vestel','YKBNK'=>'YKBNK - Yapi Kredi'];
...
<div id="results">
  ...
  <div class="metrics-grid">
    <div class="metric-card">
      ...
      <div class="m-desc">On average, the model's price prediction is off by this many TRY. Lower is better.</div>
    ...
    <div class="metric-card">
      ...
      <div class="m-label">RMSE — Error in TRY</div>
      <div class="m-value" id="mRMSE">--</div>
      <div class="m-desc">The most intuitive error metric — the typical prediction error in TRY. Lower is better.</div>
  ...
  <div class="oos-note">
    <i class="fa-solid fa-flask-vial" style="color:var(--green)"></i>
    This backtest runs only on <strong style="color:var(--green)">out-of-sample data</strong> (the 20% test period). Starting with 100,000 TRY — buying when the model predicts a rise, selling when it predicts a fall.
  </div>
  <div class="trading-card">
    <div class="t-stat"><div class="t-label">Starting Capital</div><div class="t-value">100,000 TRY</div></div>
...
function runAnalysis(){
  ...
  fetch('api.php?symbol='+encodeURIComponent(sym))
    .then(r=>r.json())
    .then(data=>{
      stopSteps();
      ...
      document.getElementById('levelEntry').textContent=fmt(sig.entry_low)+' – '+fmt(sig.entry_high)+' TRY';
      document.getElementById('levelTarget').textContent=fmt(sig.target)+' TRY';
      document.getElementById('levelStop').textContent=fmt(sig.stop_loss)+' TRY';
      ...
      // ── Price estimate
      document.getElementById('predPrice').textContent=fmt(data.next_day_price)+' TRY';
      ...
      document.getElementById('predMeta').textContent='Last known: '+fmt(data.last_price)+' TRY on '+data.last_date;

      // ── Metrics
      document.getElementById('mMAE').textContent=fmt(data.metrics.mae,4)+' TRY';
      document.getElementById('mMSE').textContent=fmt(data.metrics.mse,4);
      document.getElementById('mRMSE').textContent=fmt(data.metrics.rmse,4)+' TRY';
      document.getElementById('mMAPE').textContent=fmt(data.metrics.mape,2)+'%';

      // ── Trading
      const t=data.trading;const sign=t.total_profit>=0;
      document.getElementById('tFinal').textContent=fmt(t.final_value)+' TRY';
      document.getElementById('tProfit').innerHTML='<span class="'+(sign?'profit-pos':'profit-neg')+'">'+(sign?'+':'')+fmt(t.total_profit)+' TRY</span>';
