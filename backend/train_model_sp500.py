#!/usr/bin/env python3
"""
S&P 500 LSTM + HMM Financial Forecasting Backend
Usage: python3 train_model-S&P-case.py AAPL
Outputs a single JSON object to stdout.

Performance features:
  - 12-hour local CSV cache — repeat requests skip the download entirely
  - yf.Ticker().history() preferred over yf.download() (more reliable for US stocks)
...
ax1.set_xlabel('Date', color=MUTED); ax1.set_ylabel('Price (USD)', color=MUTED)
...
ax2.set_xlabel('Date', color=MUTED); ax2.set_ylabel('Price (USD)', color=MUTED)
...
for act, idx, price in trade_log:
    if act == 'Buy':
        ...
        sp = True
ax3.set_xlabel('Day Index', color=MUTED); ax3.set_ylabel('Price (USD)', color=MUTED)
